document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateOffersState() {
      const offerCards = Array.from(offerContainer.children).filter(el => {
        return el.nodeType === 1;
      });

      if (offerCards.length === 0) {
        offerContainer.classList.add("empty");
        deleteSelectedButton.style.display = "none";
      } else {
        offerContainer.classList.remove("empty");
        deleteSelectedButton.style.display = "inline-block";
      }
    }

    const observer = new MutationObserver(updateOffersState);
    observer.observe(offerContainer, { childList: true, subtree: false });

    window.addEventListener("load", updateOffersState);
    setTimeout(updateOffersState, 200);
    setTimeout(updateOffersState, 600);
